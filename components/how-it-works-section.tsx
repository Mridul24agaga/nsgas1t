  ...
  return (
    <section className="py-16 bg-white">
      <div className="container px-4 mx-auto">
        ...
        {/* Modified this container to center the cards */}
        <div className="mb-16">
          {/* For large screens: display as a centered flex row */}
          <div className="hidden lg:flex justify-center gap-6 flex-wrap">
            {steps.map((step, index) => (
              <div
                key={index}
                className="rounded-2xl bg-white overflow-hidden border-2 border-indigo-100 shadow-lg hover:shadow-xl transition-all duration-300 group w-[400px] flex-shrink-0"
              >
                <div className="p-8 text-center">
                  <h3 className={`text-2xl font-bold mb-3 ${step.textColor}`}>{step.title}</h3>
                ...
                <div className="bg-gradient-to-br from-indigo-50 to-indigo-100 h-72 relative border-t-2 border-indigo-100">
                  <Image
          ...
          {/* For smaller screens: keep the scrollable container */}
          <div className="lg:hidden overflow-x-auto pb-4">
            <div className="flex flex-nowrap gap-6 min-w-max px-4">
              {steps.map((step, index) => (
                <div
                  key={index}
                  className="rounded-2xl bg-white overflow-hidden border-2 border-indigo-100 shadow-lg hover:shadow-xl transition-all duration-300 group w-[400px]"
                >
                  <div className="p-8 text-center">
                    <h3 className={`text-2xl font-bold mb-3 ${step.textColor}`}>{step.title}</h3>
                    <p className="text-gray-600 mb-6">{step.description}</p>
